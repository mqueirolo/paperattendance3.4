...
if ($action == "edit") {
	if ($sesspageid == null) {
		...
		if ($session = $DB->get_record("paperattendance_sessionpages", array("id" => $sesspageid))){
			...
			$inputs = html_writer::div('<label for="course">Shortname del Curso:</label><div class="input-group"><input type="text" class="form-control" id="course" placeholder="2113-V-ECO121-1-1-2017"><div class="input-group-btn"><button id="sn" type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#shortnamemodal">?</button></div></div>',"form-group", array("style"=>"float:left; margin-left:10%"));
